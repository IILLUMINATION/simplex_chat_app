...
class TanglexService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      for (var attempt = 0;
          attempt < 5 && _isDbLocked(initResult.response);
          attempt++) {
        _appendLog('DB is locked, retrying init (${attempt + 1}/5)...');
      ...
      if (_isDbLocked(initResult.response)) {
        // Hot-restart в режиме разработки не должен уходить во временную папку,
        // если основная БД заблокирована предыдущим инстансом. Ждем и пробуем снова.
        _appendLog('DB still locked after retries. Forcing unlock/wait...');
        for (var attempt = 0; attempt < 5; attempt++) {
          _native.stopEventLoop();
          await Future<void>.delayed(const Duration(milliseconds: 1000));
          initResult = await _attemptInit(pathToUse, passphrase);
          if (initResult.ok) break;
        }
      }

      if (_isDbLocked(initResult.response)) {
        final hotPath =
            '${docsDir.path}/tanglex_data'; // Используем всегда основной путь!
        _appendLog(
          'DB still locked. Retrying main path: $hotPath',
        );
        pathToUse = hotPath;
        initResult = await _attemptInit(pathToUse, passphrase);
        _appendLog('migrateInitKey (forced main): ok=${initResult.ok}');
      }
